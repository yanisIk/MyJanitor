Meteor.methods({
	...
	'Tickets.edit': function(ticketId, modifier){
      ...
      if(!this.userId){
        throw new Meteor.Error(403, "You can't edit a ticket without being logged in as a renter or admin");
      }
      if(!Roles.userIsInRole(this.userId, ['renter', 'admin'])){
          throw new Meteor.Error(403, "You should be an admin or renter to edit a ticket on a ticket");
      }
      var ticket = Tickets.findOne(ticketId); 
      if(!ticket){
          throw new Meteor.Error(500, "Cannot edit ticket: ticket doesn't exist");
      }
      if(Roles.userIsInRole(this.userId, ['renter'])){
          if(ticket.userId !== this.userId){
            throw new Meteor.Error(403, "Cannot edit ticket: your are not the author of this ticket");
          } 
      }
      Tickets.update({'_id': ticketId}, {$set: {'content': modifier}});
	},
...
deleteTicketAndComments = function(ticketId){
  Tickets.remove(tickedId);
  TicketComments.remove({'ticketId': ticketId});
}